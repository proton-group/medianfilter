...
#pragma pack(pop)
	...
	public:
		...
		Bitmap(const std::vector<Pixel> data, const std::int32_t width, const std::int32_t height)
			:
			m_pixels(data),
			m_width(width),
			m_height(height)
	...
	public:  /* Accessors */
		...
		*/
		...
		const std::vector<Pixel> GetPixelsMas()
		{
			return m_pixels;
		}
		...
		*/
		std::string Load(const std::string& filename)
		{
			...
			if (std::ifstream ifs{ filename, std::ios::binary })
			{
				...
				{
					ifs.close();
					return ("Bitmap::Load(\"" + filename + "\"): Unrecognized file format.\n");
				...
				{
					ifs.close();
					return ("Bitmap::Load(\"" + filename + "\"): Only 24 bits per pixel bitmaps supported.\n");
				}
				...
				return ("Bitmap::Load(\"" + filename + "\"): Failed to load bitmap pixels from file.\n");
		}
